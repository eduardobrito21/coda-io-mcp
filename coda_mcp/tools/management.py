...
def register(mcp: FastMCP) -> None:
    @mcp.tool()
    async def delete_doc(doc_id: str, coda_api_key: str = CodaApiKeyDependency) -> str:
        """Permanently delete a Coda doc by ID. This cannot be undone."""
        await coda_client.docs.delete_doc(doc_id, api_key=coda_api_key)
    ...
    async def delete_page(
        ...
        await coda_client.doc_structure.delete_page(doc_id, page_id, api_key=coda_api_key)
    ...
    async def list_folders(
        ...
        data = await coda_client.folders.list_folders(query, api_key=coda_api_key)
        return data.items

    @mcp.tool()
    async def patch_doc(
        doc_id: str, title: str = "", icon_name: str = "", coda_api_key: str = CodaApiKeyDependency
    ) -> str:
        ...
        await coda_client.docs.patch_doc(doc_id, body, api_key=coda_api_key)
